...
WHERE 
    PAYMENT_HISTORY.MEMBER_ID = MEMBER.ID AND
    PAYMENT_HISTORY.PRODUCT_ID = PRODUCT.ID AND
    TO_CHAR(PAYMENT_HISTORY.ORDER_DATA, 'YYYYMM') = '202001' AND
    ROWNUM < 2
ORDER BY
    PAYMENT_HISTORY.ORDER_PRICE;
...
SELECT 
    TO_CHAR(ORDER_DATA, 'YYYYMMDD'), SUM(ORDER_PRICE)
FROM
    PAYMENT_HISTORY
GROUP BY
    TO_CHAR(ORDER_DATA, 'YYYYMMDD')
ORDER BY
    TO_CHAR(ORDER_DATA, 'YYYYMMDD');
...
-- oracle IF(https://coding-factory.tistory.com/451),
-- SUBSTR(https://docs.oracle.com/cd/B19306_01/server.102/b14200/functions162.htm),
